    ...
    success: function (result) {
        ...
        $.each(result.results, function (key, val) {
            listpokemon += `<tr>
                                <td>${key+1}</td>
                                <td>${val.name}</td>
                                <td><button type="button" class="btn btn-primary" onclick="launchModal('${val.url}');" data-url="${val.url}" data-toggle="modal" data-target="#modalP">Detail</button></td>
                            </tr>`;
        });
